...
type NacosGrpcClient struct {
	...
	serviceMap    AllDomsMap
}

func NewNacosGrpcClient(namespaceId string, serverHosts []string) (*NacosGrpcClient, error) {
	...
	nacosGrpcClient.serviceMap = AllDomsMap{}
	nacosGrpcClient.serviceMap.Data = make(map[string]bool)
	nacosGrpcClient.serviceMap.DLock = sync.RWMutex{}
	nacosGrpcClient.serviceMap.CacheSeconds = 20

	go nacosGrpcClient.asyncGetAllServicesInfo()

	return &nacosGrpcClient, err
}

func (ngc *NacosGrpcClient) asyncGetAllServicesInfo() {
	for {
		services := ngc.GetAllServicesInfo()
		time.Sleep(time.Duration(ngc.serviceMap.CacheSeconds) * time.Second)

		ngc.serviceMap.DLock.Lock()
		for _, service := range services {
			if !ngc.serviceMap.Data[service] {
				ngc.serviceMap.Data[service] = true
			}
		}
		ngc.serviceMap.DLock.Unlock()
	}
}

func (ngc *NacosGrpcClient) SelectOneHealthyInstances(serviceName string) (*model.Instance, error) {
	defer ngc.serviceMap.DLock.RUnlock()
	ngc.serviceMap.DLock.RLock()
	if !ngc.serviceMap.Data[serviceName] {
		return nil, nil
	}
	return ngc.grpcClient.SelectOneHealthyInstance(vo.SelectOneHealthInstanceParam{ServiceName: serviceName})
}
